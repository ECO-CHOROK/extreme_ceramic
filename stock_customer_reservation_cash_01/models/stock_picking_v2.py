...
class StockPickingV2Reservation(models.Model):
    ...
    def _sync_sale_order_after_delivery(self, order, original_partner, old_qty_by_line):
        ...
        # The remainder is still the same reservation (same physical stock,
        # same original customer) split onto a new order. Keep it traceable
        # instead of leaving it orphaned from the reservation and its cash
        # collections.
        reservation = order.reservation_ids.filtered(lambda r: r.reservation_state == "reserved")[:1]
        if reservation:
            reservation.linked_sale_order_ids = [(4, new_order.id)]
            new_order.reservation_id = reservation.id
            new_order.reservation_state = "reserved"
