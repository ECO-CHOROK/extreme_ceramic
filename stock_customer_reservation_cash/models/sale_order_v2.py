...
class SaleOrderV2Reservation(models.Model):
    ...
    reservation_ids = fields.One2many("sale.customer.reservation", "sale_order_id", readonly=True)
    ...
    def action_confirm(self):
        ...
        for order in self:
            if order.reservation_state != "reserved" or not order.reservation_id:
                continue
            reservation_location = (
                order.reservation_id.internal_picking_id.location_dest_id
                or order.partner_id.property_reservation_location_id
            )
            if not reservation_location:
                continue
            delivery_pickings = order.picking_ids.filtered(
                lambda p: p.picking_type_code == "outgoing" and p.state not in ("done", "cancel")
            )
            for picking in delivery_pickings:
                if picking.location_id == reservation_location:
                    continue
                picking.location_id = reservation_location.id
